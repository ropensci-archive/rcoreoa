...
#' @details \code{core_repos_search} does the HTTP request and parses, while
#' \code{core_repos_search_} just does the HTTP request, gives back JSON as a character
#' string
#'
#' A POST method is allowed on this route, but it's not supported yet.
#' @references \url{https://core.ac.uk/docs/#!/repositories/search}
#' @examples \dontrun{
...
#' }
core_repos_search <- function(query, page = 1, limit = 10, key = NULL, parse = TRUE, ...) {
...
core_repos_search_ <- function(query, page = 1, limit = 10, key = NULL, ...) {
  must_be(limit)
  core_GET(path = file.path("repositories/search", query), key, list(page = page, pageSize = limit), ...)
}
